...
Запускается в докере вместе со всем остальным сервисом, см. README проекта
...
`WORKER_TOKEN` — **тот же** секрет, что на сервисах `workers/stt`, `llm`, `redact` (`Authorization: Bearer …`). Если на воркере токен задан, а в runner — пустой, STT ответит **401**. В Docker задайте переменные в `.env` рядом с `docker-compose.yml`.
