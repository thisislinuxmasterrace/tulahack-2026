...
Для **pydub** в системе нужен **ffmpeg** (декодирование входа и экспорт в выбранный формат).

**Python 3.13+:** в stdlib убран модуль `audioop`. Нужен пакет **`audioop-lts`** (`pip install -r requirements.txt` подтянет его сам).
