...
3. passport: серия и номер часто идут отдельными непрерывными кусками («серия 2720, номер 857-815»). Это НЕ парадокс: верни **две** (или больше) записи с entity_type passport — по одной на каждый такой кусок, например original «2720» и отдельно original «857-815» (ровно как в тексте). Не пытайся собрать одну original через слова «номер», запятые или пробелы между частями.
