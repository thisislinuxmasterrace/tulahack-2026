...
2. original — подстрока из текста целиком: от первого до последнего символа чувствительного фрагмента в этом месте. Без префиксов вроде «мой номер», «звоните», но сам номер/адрес возьми непрерывным куском как в тексте.
3. passport: только серия/номер, не целое предложение. inn: только цифры ИНН. phone: original — ОДНА непрерывная цитата из расшифровки, включающая весь номер; если в тексте сказано «Плюс 7…» или «+7…», в original должны быть и «Плюс»/«+», и все цифры, и пробелы/дефисы между ними — НЕ сокращай до одних цифр, иначе маскирование сломается. Разговорные тестовые наборы вроде «1-2-3» без признаков телефона не считай номером телефона.
4. Повторы: если одно и то же значение встречается несколько раз (повтор собеседника, «ещё раз», уточнение) — включай КАЖДОЕ вхождение в список. Не пропускай повторы. Если в разных местах текст записан по-разному (пробелы, дефисы, «плюс» словом и т.д.), для каждой дословной подстроки нужна отдельная запись в entities с тем original, как она выглядит именно в этом месте.
5. Не выдумывай данные; при сомнении не включай.
6. Если персональных данных в тексте НЕТ, верни пустой массив: {{"entities": []}}.
