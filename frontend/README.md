...
| Маршрут | Описание |
|---------|----------|
| `/` | Главная: сценарий, ссылки на загрузку и пример экрана |
| `/upload` | Загрузка аудио (нужна авторизация) |
| `/result/:uploadId` | Результат обработки с сервера |
| `/demo` | Пример экрана на **мок-данных**: транскрипт, плеер, отчёт, журнал обработки |
...
- `src/mocks/demoSession.ts`, `demoProcessingEvents.ts` — демо-данные для `/demo`
