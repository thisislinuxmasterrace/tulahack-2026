...
async function submit() {
  ...
  loading.value = true
  try {
    const data = await loginRequest(username.value.trim(), password.value)
    setTokens(data.access_token, data.refresh_token)
    const redir = (route.query.redirect as string) || '/upload'
...
<template>
  <div class="auth">
    <PageIntro title="Вход" subtitle="Логин и пароль, которые вы задавали при регистрации." />

    <UiCard title="Аккаунт">
